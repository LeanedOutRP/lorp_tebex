...
export default function CartDrawer({isOpen, onClose}: Props) {
    ...
    return (
        <Drawer opened={isOpen} onClose={onClose} position="right" size="md" offset={8} radius="sm" withCloseButton={true} styles={{content: {backgroundColor: "#111111"}, header: {backgroundColor: "#1a1a1a", borderBottom: "1px solid rgba(135, 41, 243, 0.2)", padding: "1.5rem"}, body: {padding: "1.5rem"}}} title={<Group gap="sm"><ActionIcon size="lg" radius="xl" color="violet" variant="light"><IconShoppingCart size={20}/></ActionIcon><div><Text fw={700} size="lg">Shopping Cart</Text><Text size="xs" c="dimmed">{items.length} {items.length === 1 ? "item" : "items"}</Text></div></Group>}>
            {items.length === 0 ? (
                <Center h="60vh">
                ...
                <Stack gap="xl">
                    <Stack gap="md">
                        {items.map((item) => (
                            <Paper key={item.package.id} p="md" radius="sm" withBorder style={{borderColor: "#2a2a2a", backgroundColor: "#1a1a1a", transition: "all 0.2s ease"}} onMouseEnter={(e) => {e.currentTarget.style.borderColor = "rgba(135, 41, 243, 0.3)"}} onMouseLeave={(e) => {e.currentTarget.style.borderColor = "#2a2a2a"}}>
                    ...
                    <Paper p="md" radius="sm" style={{border: "1px solid rgba(135, 41, 243, 0.2)", backgroundColor: "#1a1a1a"}}>
                        <Group justify="space-between" align="center">
                            <div>
                            ...
                            <Text size="xl" fw={900} c="#9b3dff">
                                ${totalPrice.toFixed(2)}
                            </Text>
                        </Group>
                    </Paper>
                    <Stack gap="sm">
                        <Button fullWidth size="lg" color="violet" onClick={handleCheckout}>
                            Proceed to Checkout
                        </Button>
