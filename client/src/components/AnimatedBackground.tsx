    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            trailsRef.current = trailsRef.current.filter((trail) => {
                trail.life -= 0.02
                if (trail.life > 0) {
                    ctx.fillStyle = `rgba(135, 41, 243, ${trail.life*0.3})`
                    ctx.beginPath()
            ...
            nodesRef.current.forEach((node, i) => {
                ...
                ctx.fillStyle = `rgba(135, 41, 243, ${0.4*pulse})`
                ctx.beginPath()
                ...
                nodesRef.current.forEach((otherNode, j) => {
                    ...
                    if (dist < 150) {
                        ...
                        let lineColor = `rgba(135, 41, 243, ${opacity})`
                        ...
                        if (mouseDist < 150) {
                            ...
                            lineColor = `rgba(135, 41, 243, ${opacity + mouseInfluence*0.3})`
                        }
                        ...
                        if (dist < 140 && i % 3 === 0) {
                            nodesRef.current.forEach((thirdNode, k) => {
                                ...
                                if (d1 < 140 && d2 < 140) {
                                    ...
                                    ctx.fillStyle = `rgba(135, 41, 243, ${triOpacity*0.025})`
                                    ctx.beginPath()
                                    ctx.moveTo(node.x, node.y)
                                    ctx.lineTo(otherNode.x, otherNode.y)
            ...
            if (mouseRef.current.x > 0 && mouseRef.current.y > 0) {
                ...
                gradient.addColorStop(0, "rgba(135, 41, 243, 0.05)")
                gradient.addColorStop(0.5, "rgba(135, 41, 243, 0.02)")
                gradient.addColorStop(1, "rgba(135, 41, 243, 0)")
                ctx.fillStyle = gradient
                ctx.beginPath()
                ctx.arc(mouseRef.current.x, mouseRef.current.y, 100, 0, Math.PI*2)
                ctx.fill()
                const ringPulse = Math.sin(timeRef.current*3)*10 + 60
                ctx.strokeStyle = `rgba(135, 41, 243, ${0.1*(1 - ringPulse/70)})`
                ctx.lineWidth = 2
                ctx.beginPath()
            ...
            for (let i = 0; i < floatingCount; i++) {
                ...
                ctx.fillStyle = `rgba(135, 41, 243, ${0.1 + Math.sin(timeRef.current + i)*0.05})`
                ctx.beginPath()
