...
export default function Header() {
    ...
    return (
        <Box component="header" style={{position: "sticky", top: 0, left: 0, right: 0, zIndex: 100, backgroundColor: "rgba(17, 17, 17, 0.7)", backdropFilter: "blur(20px) saturate(180%)", WebkitBackdropFilter: "blur(20px) saturate(180%)", borderBottom: "1px solid rgba(135, 41, 243, 0.2)", boxShadow: "0 8px 32px 0 rgba(0, 0, 0, 0.37)"}}>
            <Group h={70} px="md" justify="space-between" align="center">
                <Link to="/" style={{textDecoration: "none"}}>
                    <Group gap="sm">
                        <img src="/logo.png" alt="Leaned Out Store" style={{height: "45px", width: "auto", objectFit: "contain"}}/>
                    </Group>
                </Link>
                <Group gap="xs" visibleFrom="sm" align="center">
                    {navItems.map((item) => (
                        <Button key={item.path} component={Link} to={item.path} variant={isActive(item.path) ? "filled" : "subtle"} styles={{root: {...(isActive(item.path) ? {background: "#9b3dff", color: "#111111", fontWeight: 600} : {color: "#FFFFFF", background: "transparent"}), "&:hover": {background: isActive(item.path) ? "#9b3dff" : "#1a1a1a"}}}}>
                            {item.label}
                        </Button>
                    ))}
                </Group>
                ...
                <Burger opened={mobileMenuOpen} onClick={() => setMobileMenuOpen(!mobileMenuOpen)} hiddenFrom="sm" color="#9b3dff"/>
            </Group>
            <Drawer opened={mobileMenuOpen} onClose={() => setMobileMenuOpen(false)} position="right" title="Menu" padding="md" hiddenFrom="sm">
                <Stack>
                    {navItems.map((item) => (
                        <Button key={item.path} component={Link} to={item.path} variant={isActive(item.path) ? "filled" : "subtle"} onClick={() => setMobileMenuOpen(false)} fullWidth styles={{root: {...(isActive(item.path) ? {background: "#9b3dff", color: "#111111", fontWeight: 600} : {color: "#FFFFFF", background: "transparent"})}}}>
                            {item.label}
                        </Button>
                    ))}
                    {!discordUser && !cfxUser ? (
