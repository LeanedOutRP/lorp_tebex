            ...
            <Container size="xl" py={20}>
                <Stack gap="md" mb={20}>
                    <Paper p="sm" radius="sm" style={{background: "#1a1a1a", border: "1px solid #2a2a2a"}}>
                        <Group gap="md">
                            <TextInput placeholder="Search products..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} leftSection={<IconSearch size={20}/>} radius="sm" flex={1} styles={{input: {background: "#111111", border: "1px solid #2a2a2a", color: "#FFFFFF", "&:focus": {borderColor: "#9b3dff"}}}}/>
                            <Menu opened={filtersOpen} onChange={setFiltersOpen} position="bottom-end" width={400} shadow="xl" styles={{dropdown: {background: "#1a1a1a", border: "1px solid #2a2a2a", padding: 0}}}>
                                <Menu.Target>
                                    <Button variant="outline" leftSection={<IconFilter size={20}/>} radius="sm" styles={{root: {borderColor: activeFiltersCount > 0 ? "#9b3dff" : "#2a2a2a", color: activeFiltersCount > 0 ? "#9b3dff" : "#FFFFFF", background: activeFiltersCount > 0 ? "rgba(135, 41, 243, 0.1)" : "transparent", "&:hover": {background: "rgba(135, 41, 243, 0.1)", borderColor: "#9b3dff"}}}}>
                                            ...
                                            <Box>
                                                <Group mb="sm" justify="space-between">
                                                    ...
                                                    {selectedTags.length > 0 && (
                                                        <Text size="xs" c="#9b3dff">
                                                            {selectedTags.length} selected
                                                        </Text>
                                                    )}
                                                </Group>
                                                <MultiSelect placeholder={allTags.length === 0 ? "No tags available" : "Select tags..."} data={allTags} value={selectedTags} onChange={setSelectedTags} searchable clearable radius="sm" disabled={allTags.length === 0} styles={{input: {background: "#111111", border: "1px solid #2a2a2a", color: "#FFFFFF", "&:focus": {borderColor: "#9b3dff"}}, pill: {background: "#9b3dff", color: "#111111"}, dropdown: {background: "#111111", border: "1px solid #2a2a2a"}, option: {color: "#FFFFFF", fontSize: "13px", "&:hover": {background: "#2a2a2a"}}}}/>
                                            </Box>
                                            <Divider color="#2a2a2a"/>
                                            <Box>
                                                <Text fw={600} c="#FFFFFF" mb="sm" size="sm">
                                                ...
                                                <Stack gap="sm">
                                                    <Checkbox label="Show only free products" checked={showOnlyFree} onChange={(e) => setShowOnlyFree(e.currentTarget.checked)} styles={{label: {color: "#FFFFFF", fontSize: "14px"}, input: {"&:checked": {background: "#9b3dff", borderColor: "#9b3dff"}}}}/>
                                                    <Checkbox label="Show only discounted products" checked={showOnlyDiscounted} onChange={(e) => setShowOnlyDiscounted(e.currentTarget.checked)} styles={{label: {color: "#FFFFFF", fontSize: "14px"}, input: {"&:checked": {background: "#9b3dff", borderColor: "#9b3dff"}}}}/>
                                                </Stack>
                                            </Box>
                                            <Divider color="#2a2a2a"/>
                    ...
                    <Group gap="sm" justify="center" style={{flexWrap: "wrap"}}>
                        <Button key="all" variant={activeCategory === "all" ? "filled" : "outline"} onClick={() => setActiveCategory("all")} radius="xl" size="sm" styles={{root: {background: activeCategory === "all" ? "#9b3dff" : "transparent", color: activeCategory === "all" ? "#111111" : "#FFFFFF", borderColor: activeCategory === "all" ? "#9b3dff" : "#2a2a2a", "&:hover": {background: activeCategory === "all" ? "#8325dbff" : "rgba(135, 41, 243, 0.1)", borderColor: "#9b3dff"}}}}>
                            All Products
                        </Button>
                            ...
                            if (hasSubcategories) {
                                return (
                                    <Menu key={category.id} position="bottom-start" shadow="xl" width={200}>
                                        <Menu.Target>
                                            <Button variant="outline" radius="xl" size="sm" rightSection={<IconChevronDown size={14}/>} styles={{root: {background: "transparent", color: "#FFFFFF", borderColor: "#2a2a2a", "&:hover": {background: "rgba(135, 41, 243, 0.1)", borderColor: "#9b3dff"}}}}>
                                                {category.name}
                                            </Button>
                                        </Menu.Target>
                                        <Menu.Dropdown style={{background: "#1a1a1a", border: "1px solid #2a2a2a"}}>
                                            {category.subcategories!.map((subcat) => (
                                                <Menu.Item key={subcat.id} onClick={() => setActiveCategory(subcat.id)} styles={{item: {color: activeCategory === subcat.id ? "#9b3dff" : "#FFFFFF", background: activeCategory === subcat.id ? "rgba(135, 41, 243, 0.1)" : "transparent", "&:hover": {background: "#2a2a2a"}}}}>
                                                    {subcat.name}
                                                </Menu.Item>
                                            ))}
                                        </Menu.Dropdown>
                    ...
                    <>
                        ...
                        <Box className="products-grid">
                            {paginatedPackages.map((pkg) => {
                                ...
                                return (
                                    <Paper key={pkg.id} radius="sm" style={{background: "#1a1a1a", border: "1px solid #2a2a2a", overflow: "hidden", cursor: "pointer", transition: "all 0.3s ease"}} onMouseEnter={(e) => {e.currentTarget.style.transform = "translateY(-4px)", e.currentTarget.style.borderColor = "#9b3dff", e.currentTarget.style.boxShadow = "0 8px 24px rgba(135, 41, 243, 0.15)"}} onMouseLeave={(e) => {e.currentTarget.style.transform = "translateY(0)", e.currentTarget.style.borderColor = "#2a2a2a", e.currentTarget.style.boxShadow = "none"}} onClick={() => navigate(`/product/${pkg.id}`)}>
                                        <Box style={{ position: "relative", width: "100%" }}>
                                            ...
                                            {isNew && (
                                                <Badge variant="filled" size="sm" radius="sm" style={{position: "absolute", top: 8, right: 8, background: "#9b3dff", color: "#111111", fontWeight: 700, fontSize: "9px", letterSpacing: "0.5px"}}>
                                                    NEW
                                                </Badge>
                                            )}
                                            {pkg.sale?.active && (
                                        ...
                                        <Stack gap={6} p={8}>
                                            ...
                                            <Group justify="space-between" mt={4}>
                                                ...
                                                {isInCart(pkg.id) ? (
                                                    ...
                                                    <Button size="xs" radius="sm" leftSection={<IconShoppingCart size={14}/>} onClick={(e) => {e.stopPropagation(), handleAddToCart(pkg)}} styles={{root: {background: "#9b3dff", color: "#111111", height: "24px", fontSize: "0.75rem", "&:hover": {background: "#a8db25"}}}}>
                                                        Add
                                                    </Button>
                                                )}
                                            </Group>
                        ...
                        {totalPages > 1 && (
                            <Center mt="xl">
                                <Pagination total={totalPages} value={currentPage} onChange={setCurrentPage} color="violet" size="lg" radius="sm" withEdges styles={{control: {"&[data-active]": {background: "#9b3dff", color: "#111111", fontWeight: 700}, "&:not([data-active])": {background: "#1a1a1a", border: "1px solid #2a2a2a", color: "#FFFFFF", "&:hover": {background: "#2a2a2a", borderColor: "#9b3dff"}}}}}/>
                            </Center>
                        )}
                    </>
                )}
                ...
                <Stepper active={authStep} onStepClick={setAuthStep} className={styles.authStepper} styles={{stepIcon: {borderColor: "#2a2a2a", background: "#1a1a1a"}, stepLabel: {color: "#FFFFFF"}, stepDescription: {color: "#666666"}, separator: {background: "#2a2a2a"}}}>
                        ...
                        <Stack gap="md" mt="xl">
                            ...
                            {cfxUser ? (
                                <Paper p="md" radius="sm" style={{background: "#1a1a1a", border: "1px solid #2a2a2a"}}>
                                    <Group>
                                        <IconCheck size={24} color="#9b3dff"/>
                                        <div>
                                ...
                                <Button fullWidth size="lg" radius="sm" leftSection={<IconUser size={20}/>} onClick={handleCFXAuth} styles={{root: {background: "#9b3dff", color: "#111111", "&:hover": {background: "#6525dbff"}}}}>
                    ...
                    <Stepper.Completed>
                        <Stack gap="md" mt="xl" align="center">
                            <IconCheck size={48} color="#9b3dff"/>
                            ...
                            <Button size="lg" radius="sm" onClick={handleAuthComplete} styles={{root: {background: "#9b3dff", color: "#111111", "&:hover": {background: "#7425dbff"}}}}>
                                Continue
                            </Button>
                        </Stack>
                    </Stepper.Completed>
