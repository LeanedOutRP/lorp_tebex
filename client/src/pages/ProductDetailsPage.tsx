...
export default function ProductDetailsPage() {
                ...
                <Box>
                    <SimpleGrid
                        ...
                        {tab.images.map((img, idx) => (
                            <Paper key={idx} radius="sm" onClick={() => setSelectedImage(img)} style={{background: "#1a1a1a", border: "1px solid #2a2a2a", overflow: "hidden", cursor: "pointer", transition: "all 0.3s ease"}} onMouseEnter={(e) => {e.currentTarget.style.transform = "scale(1.02)", e.currentTarget.style.borderColor = "#9b3dff"}} onMouseLeave={(e) => {e.currentTarget.style.transform = "scale(1)", e.currentTarget.style.borderColor = "#2a2a2a"}}>
    ...
    const handleAddToCart = () => {
        ...
        addItem(product, 1)
        notifications.show({title: "Added to cart", message: product.name, color: "violet"})
    }
    ...
    return (
        <Box py={60}>
            <Container size="xl">
                <Stack gap="xl">
                            ...
                            <Box style={{background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: "4px", padding: "20px"}}>
                                <Stack gap="md">
                                    <Box>
                                        {product.discount > 0 && product.price !== 0 ? (
                                            <Stack gap={4}>
                                                ...
                                                <Text size="2rem" c="#9b3dff" fw={900} style={{lineHeight: 1}}>
                                    ...
                                    {isInCart() ? (
                                        ...
                                        <Button fullWidth size="lg" radius="sm" onClick={handleAddToCart} leftSection={<IconShoppingCart size={20}/>} styles={{root: {background: "#9b3dff", color: "#111111", fontWeight: 700, "&:hover": {background: "#8325dbff"}}}}>
                                            ADD TO CART
                                        </Button>
                                    )}
                                </Stack>
                    ...
                    <Box style={{background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: "4px", padding: "24px"}}>
                        <Tabs value={activeTab} onChange={(value) => setActiveTab(value || "description")} variant="pills" radius="sm" color="violet" styles={{tab: {color: "#666666", fontWeight: 600, "&[data-active]": {background: "#9b3dff", color: "#111111"}, "&:hover:not([data-active])": {background: "#2a2a2a", color: "#FFFFFF"}}, list: {justifyContent: "center"}, panel: {paddingTop: "24px"}}}>
                            <Tabs.List mb="lg">
                                {productTabs.map((tab) => (
                    ...
                    <Box>
                        {reviewStats.total > 0 && (
                            <Box style={{background: "linear-gradient(135deg, rgba(135, 41, 243, 0.1), rgba(135, 41, 243, 0.05))", border: "1px solid rgba(135, 41, 243, 0.2)", borderRadius: "8px", padding: "32px", marginBottom: "24px"}}>
                                <Stack gap="lg" align="center">
                                    <Group gap="md" align="center">
                                        <IconStar size={32} color="#9b3dff" fill="#9b3dff"/>
                                        <Box style={{ textAlign: "center" }}>
                                            <Title order={2} size="2.5rem" c="#9b3dff" style={{lineHeight: 1}}>
                                    ...
                                    <Stack gap="xs" align="center">
                                        <Rating value={parseFloat(reviewStats.averageRating)} fractions={2} readOnly size="lg" color="violet"/>
                                    ...
                                    <Box style={{display: "flex", gap: "20px", animation: reviews.length > 2 ? "scroll 40s linear infinite" : "none", width: "max-content", paddingLeft: reviews.length > 2 ? "20px" : "0"}}>
                                        {[...Array(reviews.length > 2 ? 3 : 1)].map((_, duplicateIndex) => (
                                            reviews.map((review, index) => (
                                                <Paper key={`review-${duplicateIndex}-${review.id}-${index}`} radius="md" style={{background: "linear-gradient(135deg, #1a1a1a, #151515)", border: "1px solid #2a2a2a", transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", minWidth: "420px", maxWidth: "420px", flex: "0 0 auto", position: "relative", overflow: "hidden"}} onMouseEnter={(e) => {e.currentTarget.style.transform = "translateY(-8px) scale(1.02)", e.currentTarget.style.borderColor = "#9b3dff", e.currentTarget.style.boxShadow = "0 12px 40px rgba(135, 41, 243, 0.15)"}} onMouseLeave={(e) => {e.currentTarget.style.transform = "translateY(0) scale(1)", e.currentTarget.style.borderColor = "#2a2a2a", e.currentTarget.style.boxShadow = "none"}}>
                                                    <Box style={{position: "absolute", top: 0, left: 0, right: 0, height: "3px", background: `linear-gradient(90deg, transparent, #9b3dff, transparent)`}}/>
                                                    <Stack gap="lg" p="24px" pt="28px">
                                                        <Group justify="space-between" wrap="nowrap">
                                                            <Group gap="md">
                                                                {review.user_avatar ? (
                                                                    <Avatar src={review.user_avatar} size={56} radius="xl" style={{border: "2px solid rgba(135, 41, 243, 0.3)"}}/>
                                                                ) : (
                                                                    <Avatar size={56} radius="xl" style={{background: "linear-gradient(135deg, rgba(135, 41, 243, 0.3), rgba(135, 41, 243, 0.1))", border: "2px solid rgba(135, 41, 243, 0.3)"}}>
                                                                        <IconUser size={28} color="#9b3dff"/>
                                                                    </Avatar>
                                                                )}
                                                                <Stack gap={4}>
                                                                    ...
                                                                    <Group gap={6}>
                                                                        <Badge size="xs" variant="dot" color="violet" styles={{root: {background: "rgba(135, 41, 243, 0.1)", color: "#9b3dff", border: "1px solid rgba(135, 41, 243, 0.3)", textTransform: "none"}}}>
                                                                            Verified Purchase
                                                                        </Badge>
                                                            ...
                                                            <Box style={{background: "linear-gradient(135deg, rgba(135, 41, 243, 0.2), rgba(135, 41, 243, 0.1))", border: "1px solid rgba(135, 41, 243, 0.3)", borderRadius: "8px", padding: "8px 12px", textAlign: "center", minWidth: "60px"}}>
                                                                <Text size="xl" fw={900} c="#9b3dff" style={{ lineHeight: 1 }}>
                                                                    {review.rating}
                                                                </Text>
                                                                <Rating value={review.rating} readOnly size="xs" color="violet" mt={4}/>
                                                            </Box>
                                                        </Group>
            ...
            <Modal opened={showAuthModal} onClose={() => setShowAuthModal(false)} title={<Group><IconShoppingCart size={24} color="#9b3dff"/><Text size="lg" fw={700}>Authentication Required</Text></Group>} size="lg" centered padding="xl" radius="sm" styles={{content: {background: "#1a1a1a"}, header: {background: "#1a1a1a", borderBottom: "1px solid #2a2a2a"}, body: {background: "#1a1a1a"}}}>
                <Stack gap="xl">
                    <Stepper active={authStep} color="violet" size="sm" className={styles.authStepper}>
                        <Stepper.Step label="CFX Account" description="Required" icon={<IconUser style={{width: rem(18), height: rem(18)}}/>} completedIcon={<IconCheck style={{width: rem(18), height: rem(18)}}/>}>
                            <Stack gap="md" mt="xl">
                                ...
                                <Group justify="flex-end" mt="md">
                                    ...
                                    <Button color="violet" leftSection={<IconUser size={18}/>} onClick={handleCfxLogin} radius="sm">
                                        Login with CFX
                                    </Button>
                                </Group>
                            </Stack>
                        ...
                        <Stepper.Step label="Discord" description="Optional" icon={<IconBrandDiscord style={{width: rem(18), height: rem(18)}}/>} completedIcon={<IconCheck style={{width: rem(18), height: rem(18)}}/>}>
                            <Stack gap="md" mt="xl">
                                <Paper p="md" withBorder radius="sm" bg="dark.6">
                                    <Stack gap="xs">
                                        <Group>
                                            <IconBrandDiscord size={20} color="#9b3dff"/>
                                ...
                                <Group justify="flex-end" mt="md">
                                    ...
                                    <Button leftSection={<IconBrandDiscord size={18}/>} onClick={handleDiscordLogin} radius="sm" styles={{root: {background: "#9b3dff", color: "#111111", "&:hover": {background: "#a8db25"}}}}>
                                        Login with Discord
                                    </Button>
                                </Group>
                            </Stack>
            ...
            <Modal opened={selectedImage !== null} onClose={() => setSelectedImage(null)} size="auto" centered withCloseButton={false} padding={0} radius="md" styles={{content: {background: "transparent", boxShadow: "none", maxWidth: "95vw", maxHeight: "90vh"}, body: {padding: 0}}}>
                <Box style={{ position: "relative", display: "inline-block" }}>
                    <Button onClick={() => setSelectedImage(null)} radius="sm" size="md" style={{position: "absolute", top: 16, right: 16, zIndex: 10, background: "#9b3dff", color: "#111111", fontWeight: 700, boxShadow: "0 4px 12px rgba(135, 41, 243, 0.4)"}}>
